...
const getContents = async (props: { params: Props }) => {
    const { subject_slug, cat_slug, subcat_slug } = await props.params;
    const contents = await read_contents();
    // cat_slugでフィルタリング
    return contents.filter((content: Content) => (content.subject_slug === subject_slug) && (content.cat_slug === cat_slug) && (content.subcat_slug === subcat_slug));
}
...
type Props = Promise<{ subject_slug: string, cat_slug: string; subcat_slug: string; }>
                ...
                {contents.map((content: Content) => {
                    return (content) ? (
                        <Col key={content.id} span={12}>
                            <Typography>
                                <Link href={`/${content.subcat_slug}/${content.cat_slug}/${content.subcat_slug}/${content.id}`}><Title level={4}>{content.title}</Title></Link>
