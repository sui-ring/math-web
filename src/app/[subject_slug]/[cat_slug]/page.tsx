...
const getContents = async (props: { params: Props }) => {
    const { subject_slug, cat_slug } = await props.params;
    const contents = await read_contents();
    // cat_slugでフィルタリング
    return contents.filter((content: Content) => (content.subject_slug === subject_slug) && (content.cat_slug === cat_slug));
}
...
type Props = Promise<{ subject_slug: string, cat_slug: string; }>
